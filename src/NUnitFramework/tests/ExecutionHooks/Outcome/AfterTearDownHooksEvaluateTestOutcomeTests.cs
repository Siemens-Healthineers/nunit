...
public class AfterTearDownHooksEvaluateTestOutcomeTests
{
    ...
    public class AfterTearDownOutcomeLoggerAttribute : ExecutionHookAttribute
    {
        ...
        {
            Assert.That(_beforeHookTestResult, Is.Not.Null,
                "BeforeEveryTearDown was not called before AfterEveryTearDown.");
            ...
            var outcomeMatchStatement = tearDownTestResult.Outcome switch
            {
                ...
                { Status: TestStatus.Skipped } when hookData.Context.Test.FullName.Contains("4Ignored") =>
                    OutcomeMatched,
                { Status: TestStatus.Inconclusive } when hookData.Context.Test.FullName.Contains("4Inconclusive") =>
                    OutcomeMatched,
                { Status: TestStatus.Warning } when hookData.Context.Test.FullName.Contains("4Warning") =>
                    OutcomeMatched,
                _ => OutcomeMismatch
            };
